...
function fetch(baseUrl, stream) {
  ...
  function getChunk(start) {
    ...
    request({
      ...
      var len = features.length;
      // We could receive more items than we requested. That is not an
      // exceptional situation, and we should ask for the next chunk, just as
      // if we had received exactly the number requested.
      if (len >= COUNT) {
